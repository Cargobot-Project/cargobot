...
from pydrake.all import (AbstractValue, AngleAxis, Concatenate, DiagramBuilder,
                         ...
                         SchunkWsgPositionController, MakeMultibodyStateToWsgStateSystem, DifferentialInverseKinematicsParameters, DifferentialInverseKinematicsIntegrator,KinematicTrajectoryOptimization)
...
import pydot
from TrajectoryOptimizationSystem import *
...
def BoxSystem():
    ...
    parser.AddModels("/usr/cargobot/cargobot-project/perception/models/box_with_cameras.dmd.yaml")
    plant.Finalize()
...
def icp_pick_and_place_demo():
    ...
    traj_system = TrajectoryOptimizationSystem(plant)
    plan = builder.AddSystem(traj_system)

    kp=[1] * plant.num_positions()
    ki=[1] * plant.num_positions()
    kd=[1] * plant.num_positions()

    id_controller = builder.AddSystem(InverseDynamicsController(plant, kp, ki, kd, False))
    ...
    traj_system.set_plant_context(plant_context)
