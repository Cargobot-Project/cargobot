...
# Fix RNGs
def run_demo(box_list, box_list2):
    ...
    wh = WarehouseSceneSystem(model, meshcat, scene_path="/usr/cargobot/cargobot-project/res/demo_envs/multiagent.dmd.yaml", given_boxes=box_list, given_boxes_2 = box_list2)
    ...
    dimension = len(box_list)+ len(box_list2)
    num_of_boxes = len(box_list) + len(box_list2)
    print(num_of_boxes)
...
x = 0.1
box_list = [
            {'id': 0, 'dimensions': (f'{x}', f'{x}', f'{2*x}'), 'labels': (LabelEnum.HEAVY, LabelEnum.LOW_PRIORTY), 'color': BoxColorEnum.BLUE},
...
simulator, meshcat, visualizer = run_demo(box_list, box_list2)
