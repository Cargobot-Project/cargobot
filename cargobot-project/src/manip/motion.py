...
class Planner(LeafSystem):
    def __init__(self, plant, box_list, box_list_2, meshcat):
        LeafSystem.__init__(self)
        self.box_list = box_list
        self.box_list_2 = box_list_2
        self.truck_box_list = []
        self.truck_box_list_2 = []
        ...
        self.start_point = [-3,-1.25,0]
        self.limits = [0, 1.5, 0.7]

        self.max_dims_2 = []
        self.current_pillar_2 = [0,0]
        self.start_point_2 = [-3,0.75,0]
        self.limits_2 = [0, 1, 0.7]
    ...
    def Update(self, context, state):
        ...
        if mode == PlannerState.PICKING_BOX and np.linalg.norm(traj_X_G.GetPose(times["preplace"]).translation()- X_G.translation()) < 0.2 and self.current_box in self.box_list:
            ...
            if self.truck == 0:
                ...
                tmp_box = self.box_list_2.pop(self.box_list_2.index(self.current_box))
                tmp_box["pillar"] = self.current_pillar_2 
                self.truck_box_list_2.append(tmp_box)

            self.truck = (self.truck +1)%2
    ...
    def Plan(self, context, state):
        ...
        if self.second_mode == 0:
            if mode == PlannerState.PICKING_BOX:
                ...
                tf = RigidTransform(
                            RotationMatrix(RollPitchYaw(-np.pi / 2, 0,random_z)),
            ...
            if mode == PlannerState.PICKING_BOX:
                ...
                for placed_box in self.truck_box_list_2:
                    if pr in placed_box["labels"]:
                        continue_pillar = True
                ...
                z = 0
                if not continue_pillar:
                    current_x = self.current_pillar_2[0]
                    self.current_pillar_2 = [current_x+1, 1]
                    max_dim = max( float(self.current_box["dimensions"][0]), float(self.current_box["dimensions"][1]), float(self.current_box["dimensions"][2]))
                    self.max_dims_2.append([max_dim])
                    
                    x = self.start_point_2[0]+0.03
                    
                    for i in range(current_x):
                        x += max(self.max_dims_2[i])
                        x += 0.03
                
                    y = self.start_point_2[1]+ 0.03
                    z = float(self.current_box["dimensions"][2])/2+0.03
                    ...
                    for placed_box in self.truck_box_list_2:
                        if placed_box["pillar"] == self.current_pillar_2:
                            total_z += float(placed_box["dimensions"][2])
                    
                    x = self.start_point_2[0]+0.03
                    if self.current_pillar_2[0] >1:
                        for i in range(1, self.current_pillar_2[0]):
                            x += max(self.max_dims_2[i])
                            x += 0.03

                    max_dim = max(float(self.current_box["dimensions"][0]),float(self.current_box["dimensions"][1]),float(self.current_box["dimensions"][2]))
                    if total_z + float(self.current_box["dimensions"][2]) + 0.02 < self.limits_2[2]:
                        if max_dim > self.max_dims_2[self.current_pillar_2[0]-1][self.current_pillar_2[1]-1]:
                            self.max_dims_2[self.current_pillar_2[0], self.current_pillar_2[1]] = max_dim
                        z = self.start_point_2[2]+ total_z + float(self.current_box["dimensions"][2]) 

                    else:
                        self.current_pillar_2 = [self.current_pillar_2[0], self.current_pillar_2[1]+1]
                        self.max_dims_2[self.current_pillar_2[0]-1].append(max_dim)
                        z = self.start_point_2[2] + float(self.current_box["dimensions"][2])/2 + 0.02
                        
                    y += self.start_point_2[1]+0.03
                    if self.current_pillar_2[1] > 1:
                        for i in range(1, self.current_pillar_2[1]):
                                y += self.max_dims[self.current_pillar_2[0]-1][i]
                                y += 0.03

                print("XYZ: ", x, y, z)
                print("Pillar: ", self.current_pillar_2)
                print("Max Dims: ", self.max_dims_2)
                # Place in truck:
                X_G["place"] = RigidTransform(RollPitchYaw(-np.pi / 2, 0, 0), [x,y,z])
        ...
        AddMeshcatTarget(self.meshcat, f"target", X_PT=X_G["place"],length=0.2, radius=0.2)
